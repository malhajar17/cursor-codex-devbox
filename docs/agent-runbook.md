...
If browser sign-in fails at `localhost:1455`, inspect the intended SSH window's tunnel mapping and the local listener. A different SSH window may own 1455, causing Cursor to choose another local port for the intended devbox. Match the window's SSH authority, remote callback listener, and assigned local port before any recovery. A harmless request to `/` can establish reachability; it cannot establish authentication. Follow the [sign-in troubleshooting guide](setup.md#sign-in-returns-to-localhost-and-fails). Let the user complete the browser step; forwarding a supplied callback on their behalf requires explicit permission for that target. Never print, persist, publish, or repeatedly replay callback codes. Verify with `codex login status` on the intended host, without reading its credential file. This is independent of the pane and bundle repairs.
